...
const MAGIC = Uint8Array.from(
  "Kaydara FBX Binary\x20\x20\x00\x1a\x00".split(""),
  function (v) {
...
export function parseBinary(binary: any) {
  ...
  const magic = data
    .readUint8Array(MAGIC.length)
    .every(function (v: any, i: any) {
      return v === MAGIC[i];
    });
...
function readNode(data: any, header64: boolean) {
  ...
  const numProperties = header64
    ? Number(data.readUint64())
    : data.readUint32();
...
function readPropertyArray(data: any, reader: Function) {
  ...
  let arrayData = new binary_reader_1.BinaryReader(
    data.readUint8Array(compressedLength)
  );
  if (encoding == 1) {
    arrayData = new binary_reader_1.BinaryReader(
      pako_1.inflate(arrayData.binary)
    );
  }
