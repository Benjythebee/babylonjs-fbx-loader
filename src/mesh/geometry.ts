import { INumberDictionary, Undefinable, Nullable } from "../types";
...
export class FBXGeometry {
  ...
  public static ParseGeometries(runtime: IFBXLoaderRuntime): void {
    for (const g of runtime.geometries) {
      ...
      const skeleton = relationShips?.children.reduce<IFBXSkeleton | undefined>(
        (skeleton, child) => {
  ...
  public static Import(
    ...
    if (skeleton && vertexData.matricesIndices && vertexData.matricesWeights) {
      this._CleanWeights(
        skeleton,
        vertexData.matricesIndices as number[],
        vertexData.matricesWeights as number[]
      );
    ...
    if (model) {
      ...
      const rotationOrder = properties.find(
        (p) => p.prop(0, "string") === "RotationOrder"
      );
      if (rotationOrder) {
        eulerOrder = rotationOrder.prop(4, "string") ?? eulerOrder;
      }

      const geometricTranslation = properties.find(
        (p) => p.prop(0, "string") === "GeometricTranslation"
      );
      if (geometricTranslation) {
        translation.set(
          geometricTranslation.prop(4, "number") ?? 0,
          geometricTranslation.prop(5, "number") ?? 0,
          geometricTranslation.prop(6, "number") ?? 0
        );
      }

      const geometricRotation = properties.find(
        (p) => p.prop(0, "string") === "GeometricRotation"
      );
      if (geometricRotation) {
        rotation.set(
          geometricRotation.prop(4, "number") ?? 0,
          geometricRotation.prop(5, "number") ?? 0,
          geometricRotation.prop(6, "number") ?? 0
        );
      }

      const geometricScaling = properties.find(
        (p) => p.prop(0, "string") === "GeometricScaling"
      );
      if (geometricScaling) {
        scale.set(
          geometricScaling.prop(4, "number") ?? 1,
          geometricScaling.prop(5, "number") ?? 1,
          geometricScaling.prop(6, "number") ?? 1
        );
      }

      const matrix = Matrix.Compose(
        scale,
        FBXUtils.GetFinalRotationQuaternionFromVector(rotation),
        translation
      );
  ...
  private static _GenerateBuffers(sourceBuffers: IFBXInBuffers): IFBXBuffers {
    ...
    for (const mi in outputBuffersDict) {
      ...
      outputBuffers.materialIndices = outputBuffers.materialIndices.concat(
        ob.materialIndices
      );
      outputBuffers.matricesIndices = outputBuffers.matricesIndices.concat(
        ob.matricesIndices
      );
      outputBuffers.matricesWeights = outputBuffers.matricesWeights.concat(
        ob.matricesWeights
      );
    }
  ...
  private static _GenerateFace(face: IFBXFaceInfo): void {
    for (var i = 2; i < face.faceLength; i++) {
      // Positions
      face.outputBuffers.positions.push(
        -face.sourceBuffers.positions[face.facePositionIndexes[0]]
      );
      face.outputBuffers.positions.push(
        face.sourceBuffers.positions[face.facePositionIndexes[1]]
      );
      face.outputBuffers.positions.push(
        face.sourceBuffers.positions[face.facePositionIndexes[2]]
      );

      face.outputBuffers.positions.push(
        -face.sourceBuffers.positions[face.facePositionIndexes[(i - 1) * 3]]
      );
      face.outputBuffers.positions.push(
        face.sourceBuffers.positions[face.facePositionIndexes[(i - 1) * 3 + 1]]
      );
      face.outputBuffers.positions.push(
        face.sourceBuffers.positions[face.facePositionIndexes[(i - 1) * 3 + 2]]
      );

      face.outputBuffers.positions.push(
        -face.sourceBuffers.positions[face.facePositionIndexes[i * 3]]
      );
      face.outputBuffers.positions.push(
        face.sourceBuffers.positions[face.facePositionIndexes[i * 3 + 1]]
      );
      face.outputBuffers.positions.push(
        face.sourceBuffers.positions[face.facePositionIndexes[i * 3 + 2]]
      );
      ...
      if (face.sourceBuffers.skeleton) {
        ...
        face.outputBuffers.matricesWeights.push(
          face.faceWeights[(i - 1) * 4 + 1]
        );
        face.outputBuffers.matricesWeights.push(
          face.faceWeights[(i - 1) * 4 + 2]
        );
        face.outputBuffers.matricesWeights.push(
          face.faceWeights[(i - 1) * 4 + 3]
        );
        ...
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[(i - 1) * 4]
        );
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[(i - 1) * 4 + 1]
        );
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[(i - 1) * 4 + 2]
        );
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[(i - 1) * 4 + 3]
        );

        face.outputBuffers.matricesIndices.push(face.faceWeightIndices[i * 4]);
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[i * 4 + 1]
        );
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[i * 4 + 2]
        );
        face.outputBuffers.matricesIndices.push(
          face.faceWeightIndices[i * 4 + 3]
        );
      }
    }
  }
  ...
  private static _ParseUvs(
    node?: FBXReaderNode
  ): Undefinable<IFBXParsedGeometryData> {
    if (!node) {
      return undefined;
    }
  ...
  private static _ParseNormals(
    ...
    let indices: number[] = [];
    if (referenceType === "IndexToDirect") {
      indices = (node.node("NormalIndex") ?? node.node("NormalsIndex"))!.prop(
        0,
        "number[]"
      )!;
    }
  ...
  private static _GetData(
    ...
    polygonIndex: number,
    vertexIndex: number,
    infoObject: IFBXParsedGeometryData
  ): number[] {
