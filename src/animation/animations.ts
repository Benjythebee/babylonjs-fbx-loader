import { INumberDictionary, IStringDictionary, Undefinable } from "../types";
...
export class FBXAnimations {
  ...
  public static ParseAnimationGroups(runtime: IFBXLoaderRuntime): void {
    ...
    this._ParseAnimationCurves(
      runtime.objects,
      runtime.connections,
      curveNodesMap
    );

    const layersMap = this._ParseAnimationLayers(
      runtime.objects,
      runtime.connections,
      runtime.cachedModels,
      runtime.cachedSkeletons,
      curveNodesMap
    );
    const rawClips = this._ParseAnimationStacks(
      runtime.objects,
      runtime.connections,
      layersMap
    );
  ...
  private static _AddClip(
    ...
    if (layer.T && Object.keys(layer.T.curves).length > 0) {
      ...
      const positions = this._GetKeyFrameAnimationValues(
        positionTimes,
        curves,
        initialPosition
      );
    ...
    if (layer.R && Object.keys(layer.R.curves).length > 0) {
      ...
      const rotations = this._GetKeyFrameAnimationValues(
        rotationTimes,
        curves,
        initialRotation
      );
      ...
      const rotationQuaternions = rotations.map((r) => {
        ...
        if (preRotation || postRotation) {
          ...
          if (preRotation) {
            const pre =
              FBXUtils.GetFinalRotationQuaternionFromVector(preRotation);
            finalRotation = pre.multiply(finalRotation);
          }

          if (postRotation) {
            const post =
              FBXUtils.GetFinalRotationQuaternionFromVector(postRotation);
    ...
    if (layer.S && Object.keys(layer.S.curves).length > 0) {
      ...
      const scalings = this._GetKeyFrameAnimationValues(
        scalingTimes,
        curves,
        initialScale
      );
  ...
  private static _ParseAnimationStacks(
    ...
    layersMap: Map<number, IFBXLayerCurve[]>
  ): Map<number, IFBXClip> {
      ...
      connection.children.forEach((c, index) => {
        ...
        if (!layerCurveNodes[index]) {
          ...
          if (modelId !== undefined) {
            ...
            const modelRef = this._GetTargets(
              nodeId,
              cachedModels,
              cachedSkeletons
            )[0];
            ...
            if (properties?.length) {
              const preRotationNode = properties.find(
                (p) => p.prop(0, "string") === "PreRotation"
              );
              const postRotationNode = properties.find(
                (p) => p.prop(0, "string") === "PostRotation"
              );
            ...
            const node: IFBXLayerCurve = {
              ...
              id: nodeId,
              eulerOrder:
                modelRef?.metadata?.transformData?.eulerOrder ?? "ZYX",
            };
  ...
  private static _GetTargets(
    ...
    for (const skeletonId in cachedSkeletons) {
      ...
      const targetBone = skeleton.skeletonInstance.bones.find(
        (b) => b.id === id.toString()
      );
      if (targetBone) {
  ...
  private static _PrepareAnimationCurves(
    objects: FBXReaderNode
  ): Map<number, IFBXAnimationRawCurveNode> {
