...
import { Node, Matrix, AnimationGroup, Vector3, Quaternion, Animation, Tools as BabylonTools, Bone, TransformNode, Tools } from '@babylonjs/core'
        ...
        if (!layerCurveNodes[index]) {
          ...
          if (modelId !== undefined) {
            ...
            if (properties?.length) {
              ...
              if (preRotationNode) {
                preRotation = new Vector3(Tools.ToRadians(preRotationNode.prop(4, 'number') ?? 0), Tools.ToRadians(preRotationNode.prop(5, 'number') ?? 0), Tools.ToRadians(preRotationNode.prop(6, 'number') ?? 0))
              }

              if (postRotationNode) {
                postRotation = new Vector3(Tools.ToRadians(postRotationNode.prop(4, 'number') ?? 0), Tools.ToRadians(postRotationNode.prop(5, 'number') ?? 0), Tools.ToRadians(postRotationNode.prop(6, 'number') ?? 0))
              }
            }
